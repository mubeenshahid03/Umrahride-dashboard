...
function Model() {
const{isModalOpen,setisModalOpen,ModalData,isSpin,setisSpin,fetchallvehicles,isAddCar,setisAddCar}=useContext(carContext)    
    useEffect(() => {
  ...
  const onFinish =async (values) => {
    ...
    try {
        setisSpin(true)
        const response = await fetch(`https://umrahride-dashboard-backend.vercel.app/api/vehicles/editvehicle/${ModalData._id}`, {
            method: "PUT", 
            headers: {
            ...
            body: JSON.stringify({name:values.name,cartype:values.cartype,seats:values.seats,bags:values.bags,price:values.price,imgURL:values.imgURL,userid:"123"}),
          });
    ...
    try {
        setisSpin(true)
        const response = await fetch("https://umrahride-dashboard-backend.vercel.app/api/vehicles/addvehicle", {
            method: "POST", 
            headers: {
              "Content-Type": "application/json",
            //   "auth-token":localStorage.getItem("token")
...
setisAddCar(false)
    ...
    <Space>
      ...
      <Modal
        ...
        open={isModalOpen}
        title={<h3 style={{ textAlign: 'center' }}>Edit Details</h3>}
        onOk={handleOk}
