...
const CarState = (props) => {
  ...
  const fetchPkgs = async () => {
    try {
      const response = await fetch(
        `https://umrahride-dashboard-backend.vercel.app/api/vehicles/fetchpkgsfair/${localStorage.getItem(
          "selectedvehicleid"
        )}`,
        {
          method: "POST",
  ...
  const fetchParticularLocations = async () => {
    try {
      const response = await fetch(
        `https://umrahride-dashboard-backend.vercel.app/api/vehicles/fetchlocationsfair/${localStorage.getItem(
          "selectedvehicleid"
        )}`,
        {
          method: "POST",
      ...
      console.log(
        "error in frontend fetchallLocations in fair locations" + error
      );
    }
  };
